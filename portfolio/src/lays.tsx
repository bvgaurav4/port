...
export default function Lays() {
  ...
  useEffect(() => {
    ...
    const meshRemoval=(myMesh : THREE.Mesh)=>{
        if (myMesh) {
          if (myMesh.geometry) myMesh.geometry.dispose(); 
          if (myMesh.material) {
              if (Array.isArray(myMesh.material)) {
                  myMesh.material.forEach(mat => mat.dispose()); 
              } else {
    ...
    const populatingCanvas=(index:number,type:string,screenContext:CanvasRenderingContext2D,screenTexture:THREE.CanvasTexture)=>{
      ...
      var text = textrender(object.title,30);
      var lol=0;
      ...
      var text = textrender(object.content,23);
      for(var i =0;i<text.length;i++){
        var str=text[i];
  ...
  return <div style={{ position: 'relative' }}>
    ...
    <div style={{
      ...
      <button onClick={mainPage}>TRON</button>
    </div>
  </div>
} 